...
function readRequiredText(flags: Flags, key: string, message: string): string {
  const value = stringFlag(flags, key);
  if (!value) throw new Error(message);
  return readTextFile(value);
}

function readOptionalText(flags: Flags, key: string): string | undefined {
  const value = stringFlag(flags, key);
  return value ? readTextFile(value) : undefined;
}

function readOptionalEvent(flags: Flags): GitHubEvent | undefined {
  const path = stringFlag(flags, "event");
  return path ? readJsonFile<GitHubEvent>(path) : undefined;
}

function labelsFromFlags(flags: Flags): string[] | undefined {
...
function buildContext(flags: Flags): string {
