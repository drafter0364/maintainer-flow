...
export function classifyFile(path: string): FileCategory {
  ...
  if (securityPathPattern.test(normalized)) return "security";
  if (dependencyFiles.has(basename)) return "dependency";
  if (testPathPattern.test(normalized) || testFilePattern.test(normalized)) return "test";
  if (docsPathPattern.test(normalized) || docsFilePattern.test(normalized)) return "docs";
  if (/\.(json|ya?ml|toml|ini|conf|config\.[cm]?[jt]s)$/i.test(normalized)) return "config";
  if (/\.(ts|tsx|js|jsx|mjs|cjs|py|go|rs|java|kt|rb|php|cs|cpp|c|h)$/i.test(normalized)) return "source";
  return "other";
}

export function analyzePullRequest(input: PullRequestInput): AnalysisResult {
