...
import type { AnalysisResult, Finding, IssueInput } from "../types.js";
...
export function analyzeIssue(input: IssueInput): AnalysisResult {
...
function inferLabels(text: string): string[] {
  const labels: string[] = [];
  if (securityTerms.test(text)) labels.push("security");
  if (bugTerms.test(text)) labels.push("bug");
  if (featureTerms.test(text)) labels.push("enhancement");
  if (docsTerms.test(text)) labels.push("documentation");
  if (questionTerms.test(text)) labels.push("question");
